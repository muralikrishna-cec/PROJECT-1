...
def analyze_javascript(code: str):
    ...
    functions = []
    classes = []
    functions_map = {}  # map function name -> node_id
    ...
    def process_block(stmts, parent_id, depth=1):
        ...
        for stmt in stmts:
            ...
            if stype == "IfStatement":
                ...
                cons_body = cons.get("body", []) if cons and cons.get("type") == "BlockStatement" else ([cons] if cons else [])
                if cons_body:
                    process_block(cons_body, curr_id, depth+1)

                alt = stmt.get("alternate")
                if alt:
                    alt_body = alt.get("body", []) if alt.get("type") == "BlockStatement" else ([alt] if alt else [])
                    if alt_body:
            ...
            # While loop
                ...
                cond = get_expr_label(stmt.get("test"))
                curr_id = add_node(f"🔁 While ({cond})", "loop", line, full=str(stmt))
                edges.append({"from": parent_id, "to": curr_id})
                metrics["loops"] += 1
                metrics["cyclomatic_complexity"] += 1
            ...
            # For loop
                ...
                init = get_expr_label(stmt.get("init")) if stmt.get("init") else ""
                test = get_expr_label(stmt.get("test")) if stmt.get("test") else ""
                update = get_expr_label(stmt.get("update")) if stmt.get("update") else ""
                label = f"🔁 For ({init}; {test}; {update})"
                curr_id = add_node(label, "loop", line, full=str(stmt))
                edges.append({"from": parent_id, "to": curr_id})
                metrics["loops"] += 1
                metrics["cyclomatic_complexity"] += 1
            ...
            # DoWhile loop
            elif stype == "DoWhileStatement":
                cond = get_expr_label(stmt.get("test"))
                curr_id = add_node(f"🔁 DoWhile ({cond})", "loop", line, full=str(stmt))
                edges.append({"from": parent_id, "to": curr_id})
                metrics["loops"] += 1
                metrics["cyclomatic_complexity"] += 1
            ...
            # Expression statement (incl. calls)
                ...
                # Link function calls to known functions
                if expr.get("type") == "CallExpression":
                    callee = expr.get("callee", {})
                    if callee.get("type") == "Identifier":
                        fname = callee.get("name")
                        if fname in functions_map:
                            edges.append({"from": curr_id, "to": functions_map[fname]})

            elif stype == "EmptyStatement":
